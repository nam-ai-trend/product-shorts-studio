...
def main():
    ...
    if not script_path:
        script_candidates = glob.glob(os.path.join(folder, "*script*.md")) + glob.glob(os.path.join(folder, "*대본*.md"))
    ...
    if not script_path:
        print(f"에러: {folder} 내에 대본 MD 파일이 없습니다.")
        return
    ...
    # segment_index -> list of refined_block_index
    seg_to_blocks = {}
    ...
    # block_idx -> { seg_idx: (sub_start, sub_end) }
    block_seg_times = {}
    for seg_idx, block_indices in seg_to_blocks.items():
        seg = segments[seg_idx]
        total_duration = seg.end - seg.start
        
        # 각 블록의 해당 segment 매핑 텍스트 가중치 계산 (공백 제외 글자 수)
        weights = []
    ...
    # 3. 각 블록의 최종 start/end 계산 (참조하는 segment들의 sub-time 범위 조합)
    scenes = []
    scene_id = 1
