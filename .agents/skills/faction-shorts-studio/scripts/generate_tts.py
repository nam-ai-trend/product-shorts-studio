...
SPEED_RATE = 1.5  # 팩션 썰쇼츠 전용: 1.5배속
...
# 출력 파일명: 1.5배속 전용
OUTPUT_FILENAME = "output_1.5x.wav"
...
def main():
    ...
    if len(sys.argv) >= 2:
        ...
        # .md 파일이 존재하고 output_1.5x.wav 파일이 존재하지 않는 폴더 목록 필터링
        target_dirs = []
        for sd in subdirs:
            md_files = glob.glob(os.path.join(sd, "*.md"))
            has_output = os.path.exists(os.path.join(sd, OUTPUT_FILENAME))
        ...
        if not target_dirs:
            ...
            if subdirs:
                ...
                print("사용법: python3 generate_tts.py <폴더경로>")
    ...
    # MD 파일 탐색 (script.md를 우선 찾고, 없으면 아무 .md 파일)
    ...
    print(f"[*] 배속: {SPEED_RATE}x (팩션 썰쇼츠 전용)")
    ...
    final_wav = os.path.join(folder_path, OUTPUT_FILENAME)
